...
sigma_E = np.diag(0.01*np.ones(2))
mu_e = 0.
mu_a = 0.
chi = 1.
delta_vec =0.02*np.ones(2)
xi_k = 0.33 #*.75
...
def F(w):
    ...
    '''
    logm,nu,i,logc,logk_,logl,logf,logfk,xi_ = w[:ny] #y
    ...
    c,k_,l,f,fk = np.exp(logc),np.exp(logk_),np.exp(logl),np.exp(logf),np.exp(logfk)
    
    Uc = c**(-sigma)
    A = np.exp(nu)
    fl = A*(1-xi_k)*k_**(xi_k)*l**(-xi_k)
    Ul = -l**gamma
    ...
    ret[4] = xi - m * Uc
    ret[5] = xi_ - Xi_
    ret[6] = f - (A*k_**xi_k*l**(1-xi_k) + (1-delta)*k_)
...
def f(y):
    ...
    '''
    logm,nu,i,logc,logk_,logl,logf,logfk,xi_ = y #y
...
def Finv(YSS,z):
    ...
    k_rat = ((1/beta + delta - 1)/(A*xi_k))**(1/(xi_k-1))
    
    fl = A*(1-xi_k)*k_rat**xi_k
    Ul = -Xi*fl
    
    l = (-Ul)**(1/gamma)
    k_ = l*k_rat
    
    f = A*k_**xi_k*l**(1-xi_k) + (1-delta)*k_
    ...
    return np.vstack((
    logm,nu,i,np.log(c),np.log(k_),np.log(l),np.log(f),np.log(fk),xi_
    ))
...
def GSS(YSS,y_i,weights):
    ...
    '''
    logm,nu,i,logc,logk_,logl,logf,logfk,xi_  = y_i
    logK,logXi  = YSS
  
    c,k_,f = np.exp(logc),np.exp(logk_),np.exp(logf)
    K = np.exp(logK)      
...
def EulerResidual(y_,y):
    ...
    logm,nu,i,logc,logk_,logl,logf,logfk,xi_ = y
