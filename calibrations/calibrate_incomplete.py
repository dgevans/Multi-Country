...
def F(w):
    ...
    logK_,Theta_ = w[ny+ne+nY+nz+nv+n_p] #Z
...
def G(w):
    ...
    '''
    logm,nu,i,logc,logk_,f,fk,t1,xi_,a_,b_  = w[:ny] #y
    logK,logXi_,R_,T,T1 = w[ny+ne:ny+ne+nY] #Y
    logm_,nu_,i_= w[ny+ne+nY:ny+ne+nY+nz] #z
    ...
    ret[5] = K_ - k_
    ret[6] = a_-k_#f-c-K#implies f-c-K
...
def GSS(YSS,y_i,weights):
    ...
    return np.hstack([
    weights.dot(K - k_), weights.dot(f-c-k_),R-1/beta,T,T1
     ])
